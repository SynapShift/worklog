...
WorkLog 是一个本地优先的 AI 工作日志 PWA。它把每日碎片沉淀成项目进展、周期报告和复盘点评，适合个人工作复盘、学习成长、生活记录和阶段性目标管理。
...
- 如果要面向大量真实用户提供公开服务，建议补充更完整的安全审计、监控、风控和数据合规方案。

## 部署与数据边界

仓库包含源码、文档、数据库 schema 和示例配置。生产环境的 Cloudflare Pages 设置、D1 绑定、数据库和用户数据保留在 Cloudflare，不需要写入仓库。
